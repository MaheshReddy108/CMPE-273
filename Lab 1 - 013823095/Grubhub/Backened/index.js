...
app.post('/login', function (req, res) {
    ...
    pool.getConnection(function (err, conn) {
        if (err) {
            ...
            var sql = 'SELECT * from BUYER WHERE Email = ' + mysql.escape(req.body.email) +  'UNION SELECT * from BUYER WHERE Email = '+ mysql.escape(req.body.email);
...
app.post('/Owner-signup', function (req, res) {
    ...
    var presql = 'SELECT Buyer_id from Owner where Email = ' + mysql.escape(req.body.Email);
                ...
                if (err) {
                    ...
                    if (result[0]) {
                        ...
                        bcrypt.hash(req.body.Password, saltRounds).then(function(hash) {
                            ...
                            var sql = 'INSERT into BUYER (Fname, Lname,Res_name,Email,Res_address,Res_zip,Cuisine,Locations,Password) VALUES(' +
                            mysql.escape(req.body.FirstName) + ',' +
                            mysql.escape(req.body.LastName) + ',' +
                            mysql.escape(req.body.Email) + ',' +
                            mysql.escape(req.body.RestaurantAdress) + ',' +
...
app.get('/Buyer-profile-details', function (req, res) {
    ...
    console.log('Inside Buyer Profile Details GET!');
    console.log('Request Body:', req.body);
